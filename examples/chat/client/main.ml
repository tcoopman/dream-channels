...
module ChatChannel = struct
  ...
  let update model = function
    | `Join -> return model ~c:[Run (fun send -> 
    ...
    | `Joined message -> return {model with joined = true; messages = message :: model.messages}
    | `Received message -> return {model with messages = message :: model.messages}
    | `Text text -> return {model with text}
    | `Send -> 
